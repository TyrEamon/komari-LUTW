...
| `CRON_GAP` | Plaintext | 可选 | 每轮间隔秒，默认 30；`(ROUNDS-1)×GAP≈55` 且 ≤~55 |
| `PLAN` | Plaintext | 可选 | 设 `paid` 解除探针数量上限（付费版）；不设=免费版按公式自动限量 |
...
**直接用浏览器打开 Worker 首页 `https://<worker>.workers.dev`** —— 内置可视化控制台
（Lucide 图标 + 粒子网络背景 + 实验性排版，全程无表情符号），填表点按钮即可完成
注册/接入/保活/移除，不用手拼 URL。口令在页面底部填一次（本地保存）。

也可以继续用 URL 接口（控制台本质就是调它们），二选一：
...
**决定上限的三个数据**（都是 Cloudflare 免费版硬限制）：

| 限制 | 数值 | 说明 |
|---|---|---|
| 每账号请求数 | **10 万次/天** | 全账号所有 Worker **共享**；cron 每分钟触发 1 次，每次内部发 `1 + 分片数 × CRON_ROUNDS` 个请求 |
| 单次子请求 | **50 个/次调用** | 一个分片一次最多打 50 台，所以 `SHARD_SIZE ≤ 45`（留余量） |
| cron 最小间隔 | **1 分钟** | 靠内部多轮（`CRON_ROUNDS × CRON_GAP`）压到秒级；需 `(ROUNDS−1) × GAP ≤ 55s` |

**上限公式**（预留 1 万次/天给控制台/list/status）：

```
分片数 ≤ (90000 / 1440 − 1) / CRON_ROUNDS
探针上限 ≈ 分片数 × min(SHARD_SIZE, 45)
```

代码已**内置这个限量**：`/register`、`/setup` 新建探针时会按你设的 `SHARD_SIZE`/`CRON_ROUNDS` 算出上限，**超了直接拒绝并提示**（覆盖已有 token 不占名额）。`/status` 页顶部实时显示「当前 / 上限」。

**常见搭配对照（免费版，单账号单 worker）**：

| SHARD_SIZE | CRON_ROUNDS | CRON_GAP | 约刷新 | 探针上限 | 适用 |
|---|---|---|---|---|---|
| 45 | 2 | 30s | ~30s | **~1350** | 要数量：挂满全球 200 国还大有余 |
| 45 | 3 | 15s | ~15s | ~900 | 数量与频率兼顾 |
| 45 | 6 | 8s | ~8s | ~450 | **默认推荐**，几百台够用 |
| 45 | 16 | 3s | ~3s | ~135 | 少量探针 + 高频（1-3s 波动） |
| 40 | 25 | 2s | ~2s | ~80 | 极限高频，探针要少 |

> 规律：**数量和频率此消彼长**。想又多又快，就**多开免费账号**——每个账号一份全新的 10 万/天额度和独立 KV，容量按账号数**线性叠加**（2 账号≈2700，3 账号≈4000…），每账号一个 worker、各挂自己那批国家即可。
>
> **付费版**（$5/月）：设环境变量 `PLAN=paid` **解除数量上限**（没有 10 万/天上限，子请求 1000/次）。

**没设 `SELF_URL`/`SELF` 绑定时**：退回直接保活，一次调用打所有探针，受 50 子请求限制，免费版只能 **≤48 个**。
