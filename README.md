...
- **活值曲线**：按时间做平滑起伏 + 噪声 + 偶发尖峰，每台探针周期相位都不同，不呆板。
- **网络快、其它慢（木桶效应）**：网络上下行用**短周期 + 大噪声 + 频繁突发**，每次采样都明显不同；
  CPU/内存/负载用**长周期 + 小噪声**，缓慢漂移。所以在同一次上报里，网络看着"活"，CPU/内存看着"稳"。
- **刷新频率**：一次上报同时带所有字段，频率由 cron 决定（默认每 ~30s）。想让网络更"实时"就把间隔调小：
  - `CRON_ROUNDS=6`、`CRON_GAP=8` → 约每 8 秒一次（网络跳得勤，CPU/内存因曲线慢仍显得稳）
  - `CRON_ROUNDS × CRON_GAP` 控制在 ~50 秒内；轮数越多每天请求/读取越多。
- Komari 判在线只需 35s 内有上报，更快纯属好看。
